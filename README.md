...
- **Card-based UI** - modern dark theme with rounded card layout, colored type badges (POST, REPLY, RT), and per-tweet View links
...
- **Adaptive polling** - after several consecutive empty polls the interval automatically backs off (up to 5x, capped at 30 min) to save API credits; resets as soon as new tweets appear
...
3. Open **Settings** (gear icon) - paste your key in the **twitterapi.io Key** field - click **Save Key**
...
**Adaptive back-off:** After 3-4 consecutive polls with zero new tweets, the interval multiplies by 5x (capped at 30 min). As soon as a poll returns new tweets, it resets to your configured interval.
